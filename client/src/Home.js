...
function Dashboard() {
  ...
  // Add a new state to store countdown values for each entry
  const [countdown, setCountdown] = useState({});

  // Modify toggleStatus function to handle timer start/stop logic
  const toggleStatus = async (name, roll) => {
    try {
      ...
      if (existingEntryIndex !== -1) {
        ...
        setData2(newData2);
        // If entry is found, stop the countdown timer
        setCountdown((prevCountdown) => ({ ...prevCountdown, [name]: 0 }));
      } else {
        const newData2 = [...data2, { name, time: currentTime }];
        setData2(newData2);
        // If entry is not found, start the countdown timer for the next even entry
        if (
  ...
  // Modify getStatusIcon function to update countdown for green tick status
  ...
  // Update useEffect hook to start or stop the timer based on the current status
  useEffect(() => {
    let intervalId;
  ...
  return (
    <div className="containerDash">
      <h2>Welcome...! Attendance Management System</h2>
